...
bign C(bign x,bign y){
    bign res=1;
    for(bign i=0;i<y;i=i+1)
        res=res*(x-i);
    for(bign i=1;i<=y;i=i+1)
        res=res/i;
    return res;
}

bign A(bign x,bign y){
    bign res=1;
    for(bign i=0;i<y;i=i+1)
        res=res*(x-i);
    return res;
...
int main(){
    ...
    bign four=4;
    res=four.pow(n-3)*bign(3)*bign(3)*(n-3);
    res=res+bign(2)*four.pow(n-2)*bign(3);
    cout<<res<<endl;
    return 0;
}