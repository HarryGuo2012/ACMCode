...
#define rchild m+1,r,v<<1|1
	...
	bitset<MAX_M> one=1;
	void pushUp(int v){
		tree[v].bi=tree[v<<1].bi|tree[v<<1|1].bi;
	}
	void pushDown(int v){
		tree[v<<1].lazy=tree[v<<1|1].lazy=tree[v].lazy;
		tree[v<<1].bi=tree[v<<1|1].bi=(one<<tree[v].lazy);
		tree[v].lazy=0;
	}
	...
	bitset<MAX_M> query(int a,int b,int l=1,int r=N,int v=1){
		//cout<<a<<" "<<b<<" "<<l<<" "<<r<<" "<<v<<endl;
		if(a<=l&&r<=b){
			return tree[v].bi;
		}	
...
int main(){
	...
	while(q--){
		...
		fastIO::read1(y);
		if(op[0]=='Q'){
			bitset<MAX_M> res=seg.query(x,y);
			printf("%d\n",res.count());
		}
		else{
			fastIO::read1(z);
